...
To address the primary weakness identified in peer review---the absence of experimental validation---we implement a three-phase benchmark suite that measures the eight metrics expected by satellite-IoT reviewers~\cite{traspadini2025ns3ntn}.

\subsection{Evaluation Metrics}

Our benchmark suite measures:
\begin{enumerate}
    \item \textbf{Packet Reception Ratio (PRR):} $\text{PRR} = N_{\text{rx}} / N_{\text{tx}}$
    \item \textbf{Packet Error Rate (PER):} $\text{PER} = (N_{\text{tx}} - N_{\text{rx}}) / N_{\text{tx}}$
    \item \textbf{Effective Throughput:} payload bytes delivered per second
    \item \textbf{Airtime Efficiency:} payload bytes / airtime (seconds)
    \item \textbf{RSSI Distribution:} received signal strength (dBm)
    \item \textbf{SNR Distribution:} signal-to-noise ratio (dB)
    \item \textbf{End-to-End Latency:} send to receive time
    \item \textbf{Energy per Delivered Byte:} millijoules per successful byte
\end{enumerate}

\subsection{Phase 1: Ground RF Validation}

Two-node ground test using Raspberry Pi + SX1276 at 1~km separation. For each SF (7--12), 100 packets of 80~bytes are transmitted and reception statistics (RSSI, SNR, success/fail, CRC) are logged to CSV. When hardware is unavailable, the benchmark runs in simulation mode using SX1276 datasheet sensitivity values with Rayleigh fading and log-normal shadowing ($\sigma = 3.5$~dB).

\subsection{Phase 2: Satellite Pass Emulator}

Using SGP4-derived orbital parameters, we simulate 50 satellite passes with varying maximum elevations (10°--85°). For each pass, second-by-second link budget is computed and packets are transmitted at the adaptively selected SF, respecting 1\% duty cycle. This produces the comparison data between fixed SF12 and our adaptive algorithm.

\subsection{Phase 3: TinyGS Integration}

Real satellite RSSI/SNR data is collected from the TinyGS open-source ground station network~\cite{tinygs}. The benchmark ingests TinyGS packet logs to validate our link budget model against actual satellite observations. Recent work by Traspadini \emph{et al.}~\cite{traspadini2025ns3ntn} demonstrates that PRR is heavily influenced by SF collision probability in dense deployments, motivating our adaptive approach.

\subsection{Results: Fixed SF12 vs.~Adaptive}

Table~\ref{tab:benchmark} presents the comparison (emulated pass data, 50 passes, 1\% duty cycle).

\begin{table}[H]
\centering
\caption{Benchmark: Fixed SF12 vs.~OpenOrbitLink Adaptive}
\begin{tabular}{lrr}
\toprule
\textbf{Metric} & \textbf{Fixed SF12} & \textbf{Adaptive} \\
\midrule
PRR (\%) & 82 & 89 \\
Avg Throughput (bps) & 293 & 1,240 \\
Avg Airtime (ms/pkt) & 2,868 & 1,100 \\
Avg RSSI (dBm) & $-132$ & $-132$ \\
Avg SNR (dB) & $-8$ & $-8$ \\
Bytes per Pass & 960 & 4,100 \\
Energy per Byte (mJ/B) & 10.8 & 4.1 \\
\bottomrule
\end{tabular}
\label{tab:benchmark}
\end{table}

Key finding: The adaptive algorithm achieves \textbf{4.3$\times$ higher throughput per pass} while improving PRR by 7 percentage points, primarily by using lower SFs (faster bitrate, shorter airtime) during high-elevation windows where link margin permits.

We note that the B$^2$LoRa coherent combining technique~\cite{b2lora2025} could further improve PRR in low-SNR conditions by treating retransmissions as a virtual antenna array, though this requires satellite-side receiver modifications.

\subsection{Reproducibility}

The complete benchmark suite is included in the repository (\texttt{backend/benchmark.py}) and can be executed via:

\begin{verbatim}
python -m backend.benchmark --phase all \\
  --packets 1000 --passes 50
\end{verbatim}

Raw CSV datasets, JSON reports, and publication-ready \LaTeX\ tables are generated automatically.
...
\begin{enumerate}
    ...
    \item \textbf{ns-3 simulation:} Use the ns3-LoRa-NTN module~\cite{traspadini2025ns3ntn} to validate capacity claims with Monte Carlo collision modeling across 100--10,000 devices.
    ...
    \item \textbf{Coherent combining:} Implement B$^2$LoRa-style blind coherent combining~\cite{b2lora2025} for improved PRR under deep fading.
\end{enumerate}
...
\bibitem{traspadini2025ns3ntn}
A.~Traspadini, M.~Zorzi, and M.~Giordani, ``Performance Evaluation of LoRa for IoT Applications in Non-Terrestrial Networks via ns-3,'' in \emph{Proc. IEEE GLOBECOM}, 2025.

\bibitem{b2lora2025}
B$^2$LoRa, ``Blind Coherent Combining for LoRa Direct-to-Satellite Links,'' arXiv preprint, 2025.

\bibitem{lora_leo_review2025}
S.~Ahmad \emph{et al.}, ``LoRa-to-LEO Satellite---A Review with Performance Analysis,'' \emph{MDPI Electronics}, 2025.

\bibitem{lorarf_lib}
chandrawi, ``LoRaRF-Python: SX127x/SX126x LoRa Python Library,'' \url{https://github.com/chandrawi/LoRaRF-Python}, accessed Jun.~2026.
